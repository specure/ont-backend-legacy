...
public class QoSUtil {
    ...
    // logger instance
    private static final Logger logger = LoggerFactory.getLogger(QoSUtil.class);
            ...
            if (necessaryDataAvailable) {
                ...
                for (final QoSTestResult testResult : testResultList) {
                    ...
                    try {
                        ...
                        String errorMessage = "WARNING: QoS TestType '" + testResult.getTestType().toUpperCase(Locale.US) + "' not supported by ControlServer. Test with UID: " + testResult.getUid() + " skipped.";
     ...
     * @throws IllegalArgumentException
     */
//    public static String calculateTotalResultInPercentage(final Connection conn, final TestUuid uuid) throws SQLException {
//
//        String toReturn = "-";
//
//        // check connection
...
//                }
//
//                int totalCount = testResultList.size();
//                int failedCount = 0;
//
//                //iterate through all result entries
//                for (final QoSTestResult testResult : testResultList) {
//                    // increment by 1 if test failed
//                    if (testResult.getFailureCounter() > 0) failedCount++;
//                }
//
//                // count on succes tests in percentages
//                if (failedCount == 0) {
//                    toReturn = "100";
//                } else {
//                    toReturn = Long.toString(Math.round((double) (totalCount - failedCount) / (double) totalCount * 100.0));
//                }
//            }
//
//        }
...
//    }
    ...
    public static String calculateTotalResultInPercentage(final Connection conn, Long uid) throws SQLException {
        ...
        if (conn != null) {
            ...
            if (uid != null && uid > 0l) {
                ...
                QoSTestResultDao resultDao = new QoSTestResultDao(conn);
                List<QoSTestResult> testResultList = resultDao.getByTestUid(uid);
                if (testResultList == null || testResultList.isEmpty()) {
                    throw new UnsupportedOperationException("test " + uid + " has no QoS result list");
                }
