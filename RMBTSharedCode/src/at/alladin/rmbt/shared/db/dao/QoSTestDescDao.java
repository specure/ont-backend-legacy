...
public class QoSTestDescDao implements PrimaryKeyDao<QoSTestDesc, Long> {
    ...
    // logger instance
    private static final Logger logger = LoggerFactory.getLogger(QoSTestTypeDesc.class);

    private static final String SQL_SELECT_QOS_TEST_DESC = "SELECT uid, desc_key AS key, value, lang FROM qos_test_desc WHERE uid = ?";

    private static final String SQL_SELECT_QoS_TEST_DESC_ORDER_BY =
            "SELECT uid, desc_key as \"key\", \"value\", lang FROM qos_test_desc ORDER BY desc_key, lang";
    ...
    public HashMap<String, List<QoSTestDesc>> getAllToMapIgnoreLang() throws SQLException {
        ...
        try (PreparedStatement psGetAll = conn.prepareStatement(SQL_SELECT_QoS_TEST_DESC_ORDER_BY)) {
            logger.debug(psGetAll.toString());
            if (psGetAll.execute()) {
                try (ResultSet rs = psGetAll.getResultSet()) {
                    while (rs.next()) {
