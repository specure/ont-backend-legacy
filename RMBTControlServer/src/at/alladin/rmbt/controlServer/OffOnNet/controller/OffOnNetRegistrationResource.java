...
public class OffOnNetRegistrationResource extends OffOnNetServerResource {

    // logger instance
    private static final Logger logger = LoggerFactory.getLogger(OffOnNetRegistrationResource.class);
...
//                            && clientVersions.contains(request.optString("version")) && typeId > 0) {
                    if (!checkMeasurementServerVersion || (clientNames.contains(request.optString("client"))
                        ...
                        final Timestamp clientTimeStamp =
                                Timestamp.valueOf(new Timestamp(clientTime).toString());
                        ...
                        if (clientUid > 0) {
                            ...
                            if (request.has("measurement_server_id")
                                ...
                                try {
                                    ...
                                    if (onNet && uuid != null) {
                                        int port = request.getInt("ifport");
                                        providerId = getProviderIdByUserPort(uuid, port);
                                    }
                                } catch (JSONException | IllegalArgumentException err) {
                                    logger.error("Bad parameters from from client" + err.toString());
                                }
                            ...
                            if (errorList.getLength() == 0)
                                try {
                                    ...
                                    st.setTimestamp(i++, clientTimeStamp, timeWithZone);
                                    // duration (requested)
...
//                                            }
                                            ...
                                            if (geotime != 0 && geolat != 0 && geolong != 0) {
                                                ...
                                                final Timestamp geotstamp =
                                                        Timestamp.valueOf(new Timestamp(geotime).toString());
        ...
        // log response
        logger.debug("response: " + answer.toString());
