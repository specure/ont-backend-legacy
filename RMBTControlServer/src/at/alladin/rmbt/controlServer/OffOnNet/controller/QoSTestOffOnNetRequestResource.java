...
import at.alladin.rmbt.controlServer.OffOnNet.service.OffOnNetServerResource;
 ...
 * @author lb
 */
public class QoSTestOffOnNetRequestResource extends OffOnNetServerResource {

    // logger instance
    private static final Logger logger = LoggerFactory.getLogger(QoSTestOffOnNetRequestResource.class);

    @Post("json")
    public String request(final String entity) {

        // log request
        logger.debug("off/on net test request: " + entity);
        ...
        if (entity != null && !entity.isEmpty()) {
            ...
            try {
                ...
                boolean onNet = false;
                try {
                    onNet = request.getBoolean("on-net");
                } catch (JSONException | IllegalArgumentException err) {
                    logger.error("Bad parameter 'on-net' from from client" + err.toString());
                }
                ...
                if (onNet) {
                    try {
                        port = request.getInt("ifport");
                    } catch (JSONException | IllegalArgumentException err) {
                        logger.error("Bad parameter 'ifport' from from client" + err.toString());
                    }
                }

                UUID uuid = UUID.fromString(request.getString("uuid"));
                TestServerJsonInterface server = getQoSServerByUserID(uuid, true, false, onNet, port);
                ...
                if (server == null) {
                    ...
                    int providerId = this.getProviderIdByUserPort(uuid, port);
                    server = getNearestServer(
                            locationJson.getGeo_lat(),
                            locationJson.getGeo_long(),
                            clientIpString,
                            asCountry,
                            geoIpCountry,
                            "QoS",
                ...
                logger.debug("Nearest QoS server for country: " + geoIpCountry + ". asCountry: "
                        + asCountry + " id:" + ((TestServerJson) server).getId()
                        + " name:" + ((TestServerJson) server).getName()
                        + " address:" + ((TestServerJson) server).getAddress()
                        + " port:" + ((TestServerJson) server).getPort()
                        + " off/on net:" + onNet
                        + " provider for port: " + port);

                QoSTestObjectiveDao testObjectiveDao = new QoSTestObjectiveDao(conn);
                List<QoSTestObjective> listTestParams = testObjectiveDao.getByTestClass(((TestServerJson)server).getId(), 1);
                ...
                for (QoSTestObjective o : listTestParams) {
                    ...
                    // iterate through all keys and interpret their values if necessary;
                    while (keys.hasNext()) {
