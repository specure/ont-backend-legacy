 ...
 */
public class MapFilterOperators extends ServerResource {

    // logger instance
    private static final Logger logger = LoggerFactory.getLogger(MapFilterOperators.class);
    ...
    private static final String SQL_SELECT_PROVIDER_WITHOUT_MCCMNC = "SELECT DISTINCT" +
            "  (p.shortname) shortname, p.name AS name " +
    ...
    private static final String SQL_SELECT_PROVIDER_BOTH = "SELECT DISTINCT(p.shortname) AS shortname, p.name AS name " +
    ...
    private static final String SQL_SELECT_PROVIDER_WITH_MCCMNC = "SELECT DISTINCT(p.shortname) shortname,p.name as name "
...
//                obj.put("id_provider", rs.getLong("uid"));
